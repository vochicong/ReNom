...
def optimizer_check(optimizer = None):
    ...
    for e in range(epochs):
        ...
        if (e+1) % 100 is 0:
            print ('\033[2A')
            print ('\033[KFinished \033[1;31mEpoch #{:d} \033[1;32mfor optimizer \033[1;31m{}\033[1;32m'.format(e+1,type(optimizer)))

    print('\033[0m\033[1A')

    assert 2 - np.squeeze(model.params['w'].as_ndarray()) < 1.0, "Optimal weight value is {:d}, learned value of {:f}".format(2,np.squeeze(model.params['w'].as_ndarray()))

...
class average_unpoolnd(Node):
    ...
    @classmethod
    def _oper_cpu(cls, x, prev_pool):
        result = poolnim(prev_pool.attrs._x, x, prev_pool.attrs._kernel, prev_pool.attrs._stride, prev_pool.attrs._padding, mode="average")
        ret = cls._create_node(result)
        ret.attrs._x = x
        ret.attrs._original_x = prev_pool.attrs._x
        ret.attrs._kernel = prev_pool.attrs._kernel
    ...
    def _backward_cpu(self, context, dy, **kwargs):
        dx = imnpool(self.attrs._original_x, self.attrs._kernel, self.attrs._stride, self.attrs._padding, mode="average", alternate_input=dy)
        self.attrs._x._update_diff(context, dx)

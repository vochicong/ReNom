...
class gru(Node):
    ...
    def _backward_gpu(self, context, dy, **kwargs):
        ...
        cu.cugru_backward(get_gpu(ABC),get_gpu(y),yconc,get_gpu(u),get_gpu(hminus),db,du,dpz,dxx)
        # Calculate dx
...
class Gru(Parametrized):
    ...
    example:
        ...
        In [3]: n, d, t = (2, 3, 4)
        In [4]: x = rm.Variable(np.random.rand(n,d))
        In [5]: layer = rm.Gru(2)
        In [6]: z = 0
        In [7]: for i in range(t):
        ...:     z += rm.sum(layer(x))
        ...:
        In [8]: grad = z.grad()
        In [9]: grad.get(x)

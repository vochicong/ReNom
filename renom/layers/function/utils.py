...
def imnpool(img, kernel, stride, padding, padWith=0, mode="max", alternate_input=None):
    ...
    for batch in range(N):
        tmp = []
        for in_channel in range(in_channels):
            ret2 = place_pools(padded_image[batch, in_channel], kernel, stride, func, alternate_input=alternate_input[batch, in_channel] if alternate_input is not None else None)
            tmp.append(ret2)
        ret.append(tmp)
    ret = np.array(ret)
    return ret
...
def place_back_pools(img, kernel, stride, mode, dy, offset=0):
    ...
    for pos in generate_positions(img, stride, offset, min_space=np.array(kernel) - 1):
        slices = [slice(pos[i] if pos[i] > -1 else 0, pos[i] + kernel[i], 1) for i in range(len(img.shape))]
